...
func (s *PostgresStorage) GetItem(id int64) *model.Item {
	...
	`, id).Scan(
		...
		&i.Date, &i.Status, (*MediaLinks)(&i.MediaLinks), &i.AISummary,
	)
	if err != nil {
		log.Print(err)
		return nil
...
func (s *PostgresStorage) UpdateItemSummary(item_id int64, summary string) bool {
	_, err := s.db.Exec(`update items set ai_summary = $2 where id = $1`,
		item_id,
		summary,
	)
	if err != nil {
